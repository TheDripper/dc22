...
export const actions = {
  ...
  setUser({ commit, state }, loggedin) {
    ...
    // set myPosts
    let wp = new wpapi({
      endpoint: "https://dc22.nfshost.com/wp-json",
  ...
  async nuxtServerInit({ commit }) {
    ...
    let wp = new wpapi({
      ...
      password: "5Fy9 sFmY jHVw m7sz 4Hwc NCgn",
      auth: true,
    });
    ...
    const howitworks = await wp.pages().id(7).get();
    ...
    //   "https://dc22.nfshost.com/wp-json/wp/v2/pages/405"
    // );
    ...
    // commit("posts", posts);
    const header = await wp.pages().id(10).get();
    commit("header", header.content.rendered);
    const footer = await wp.pages().id(12).get();
